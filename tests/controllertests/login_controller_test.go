...
func TestSignIn(t *testing.T) {
	...
	samples := []struct {
		...
		{
			email:        user.Email,
			passowrd:     "password", //Note the password has to be this, not the hashed one from the database
			errorMessage: "",
		},
		{
			email:        user.Email,
			passowrd:     "Wrong password",
	...
	for _, v := range samples {
		token, err := server.SignIn(v.email, v.passowrd)
		if err != nil {
...
func TestLogin(t *testing.T) {
	...
	samples := []struct {
		...
		{
			inputJSON:    `{"email": "pet@gmail.com", "password": ""}`,
			statusCode:   422,
